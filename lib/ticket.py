"""Telemetry ticket packing helpers.

This module is intentionally CPython-compatible so the payload format can be
tested on a normal computer before running on the Raspberry Pi Pico.
"""
...
def make_ticket(user, place, sensor_id, data, observations, day, time):
    """Build the fixed 16-byte telemetry ticket."""
    ...
    if len(ticket) != 16:
        raise AssertionError("ticket must be exactly 16 bytes")
